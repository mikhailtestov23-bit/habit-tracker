...
- Vercel Cron настроен в `vercel.json` и вызывает `/api/cron/reminders` каждые 15 минут.
...
Схема включает Row Level Security и политики для будущей авторизации. Текущий MVP работает через серверный secret/service key, поэтому API routes на Vercel смогут читать и писать данные с серверной стороны.
...
- `/` — интерфейс трекера.
- `/api/state` — состояние приложения.
- `/api/export` — JSON-экспорт.
- `/api/cron/reminders` — cron endpoint.

На первом запросе приложение создаст стартовые привычки, если таблица `habits` пустая.

## 6. Что останется сделать после MVP

- Подключить Supabase Auth и заменить `local-user` на реальный `auth.uid()`.
